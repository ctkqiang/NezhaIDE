...
namespace NezhaIDE::Services {
    ...
    Disassembler::disassemble(const uint8_t *data, const size_t size, const uint64_t offset_in_file,
                              const uint64_t base_address, Model::BinaryArch arch) {
        if (!data || size == 0) {
            return std::unexpected("empty data");
        }

        cs_mode mode;
        const auto cs_arch = map_cs_arch(arch, mode);
        ...
        if (const auto err = cs_open(cs_arch, mode, &handle); err != CS_ERR_OK) {
        ...
        const auto count = cs_disasm(handle, data, size, base_address, 0, &insns);
        if (count == 0 || !insns) {
            cs_close(&handle);
